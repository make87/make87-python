...
def main():
    ...
    queryable = zenoh_interface.get_queryable("QUERYABLE_1")
    print("Queryable 1 started listening...")
    ...
    while True:
        with queryable.recv() as query:
            try:
                ...
                print(
                    f"Queryable 1 received query '{message.body}' from {message.header.entity_path}. "
                    f"Sent at {publish_dt}. Received at {received_dt}. "
                    f"Took {(received_dt - publish_dt).total_seconds():.3f} seconds."
                )

                # Create response
                header = Header(entity_path="/test/queryable_1", reference_id=1)
                header.timestamp.GetCurrentTime()

                response_message = PlainText(
                    header=header,
                    body=f"Response from Queryable 1 to: {message.body}",
                )
                message_encoded = message_encoder.encode(response_message)

                query.reply(key_expr=query.key_expr, payload=message_encoded)
                print(f"Queryable 1 responded to query: {message.body}")
            except Exception as e:
                print(f"Queryable 1 error processing query: {e}")
