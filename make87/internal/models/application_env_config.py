...
#   timestamp: 2025-09-23T21:08:29+00:00
...
class Binding(BaseModel):
    container_ip: str
    container_port: conint(ge=0)
    host_ip: str
    host_port: conint(ge=0)
...
class MultiClientServiceConfig(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    interface_name: Optional[str] = None
...
class MultiSubscriberTopicConfig(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
...
class BoundMultiClient(MultiClientServiceConfig):
    access_points: Dict[str, AccessPoint]


class BoundMultiSubscriber(MultiSubscriberTopicConfig):
    access_points: Dict[str, AccessPoint]


class BoundPeer(PeerConfig):
    access_points: Dict[str, AccessPoint]


class BoundProvider(ProviderEndpointConfig):
    binding: Optional[Binding] = None


class BoundPublisher(PublisherTopicConfig):
    binding: Optional[Binding] = None
...
class BoundServer(ServerServiceConfig):
    binding: Optional[Binding] = None
...
class InterfaceConfig(BaseModel):
    ...
    multi_clients: Optional[Dict[str, BoundMultiClient]] = None
    multi_subscribers: Optional[Dict[str, BoundMultiSubscriber]] = None
    name: str
    peers: Optional[Dict[str, BoundPeer]] = None
    providers: Dict[str, BoundProvider]
    publishers: Dict[str, BoundPublisher]
    requesters: Dict[str, BoundRequester]
    servers: Dict[str, BoundServer]
